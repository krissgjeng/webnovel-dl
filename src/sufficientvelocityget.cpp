...
//https://forums.sufficientvelocity.com/threads/an-essence-of-silver-and-steel-worm-fate-stay-night-alt-power.39043/reader/
SufficientVelocityGet::SufficientVelocityGet(std::string url)
{
    this->url = url;
...
std::string SufficientVelocityGet::getAuthorSrc(const std::string &source)
{
    using std::string;
    StringHelper str;
...
std::string SufficientVelocityGet::getTitleSrc(const std::string &source)
{
    using std::string;
    StringHelper str;
    CurlWrapper cw;
...
std::string SufficientVelocityGet::getChUrl(int ch, std::string url)
{
...
std::string SufficientVelocityGet::getCh(int ch)
{
...
int SufficientVelocityGet::GetChCount()
{
    if (chcount == -1)
    {
        using std::string;
...
std::vector<std::string> SufficientVelocityGet::getAllCh(std::function<void(int, std::string)> callback, int delayinms)
{
...
void SufficientVelocityGet::getAllCb(std::function<void(int, std::string)> callback, int delayinms)
{
    int to = GetChCount();
    getChaptersCb(1,to,callback,delayinms);
}
void SufficientVelocityGet::getChaptersCb(int from, int to, std::function<void(int, std::string)> callback, int delayinms)
{
    using std::string;
