...
std::string CurlWrapper::getSource(std::string url)
{
    std::string cacertpath= std::filesystem::current_path().string()+"/cacert.pem";
    std::string content = "";
    CURL *curl_handle;
    CURLcode res;    
    ...
    curl_easy_setopt(curl_handle, CURLOPT_CAINFO, cacertpath.c_str());
    //curl_easy_setopt(curl_handle, CURLOPT_CAPATH, cacertpath.c_str());
     ...
     */
        content = std::string(chunk.memory, chunk.size);
